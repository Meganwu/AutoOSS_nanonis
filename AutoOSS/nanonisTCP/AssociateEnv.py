...
from nanonisTCP.nanonisController import nanonisController
from nanonisTCP.utils.img_attrib import mol_property
from nanonisTCP.utils.img_conv import ResNet18, NormalNet, ResNet
from nanonisTCP.utils.net_framework import img_classifier
...
# associate_data = namedtuple('associate_data',['time','x','y','current','dI_dV','topography'])




class AssociateEnv(nanonisController):
        def __init__(self,
                ...
                total_output_folder = 'all_output',        # save images and data
                ...
                self.nanonis_controller = nanonisController(*args)
                ...
                self.task_folder='%s/%s_%s' % (total_output_folder, task_folder_prefix, current_time)
                self.save_img_large='%s/%s_%s' % (self.task_folder, save_scan_img_large_prefix, current_time)
                self.save_img_before='%s/%s_%s' % (self.task_folder, save_scan_img_before_prefix, current_time)
                self.save_img_after='%s/%s_%s' % (self.task_folder, save_scan_img_after_prefix, current_time)
                self.save_data_large='%s/%s_%s' % (self.task_folder, save_scan_data_large_prefix, current_time)
                self.save_data_before='%s/%s_%s' % (self.task_folder, save_scan_data_before_prefix, current_time)
                self.save_data_after='%s/%s_%s' % (self.task_folder, save_scan_data_after_prefix, current_time)
                self.save_vert_data='%s/%s_%s' % (self.task_folder, save_vert_data_prefix, current_time)
                self.save_vert_tip_pos='%s/%s_%s' % (self.task_folder, save_vert_tip_pos_prefix, current_time)

                # check if these folders exist
                if not os.path.exists(self.task_folder):
                        os.mkdir(self.task_folder)
                if not os.path.exists(self.save_img_large):
                        os.mkdir(self.save_img_large)
                if not os.path.exists(self.save_img_before):
                        os.mkdir(self.save_img_before)
                if not os.path.exists(self.save_img_after):
                        os.mkdir(self.save_img_after)
                if not os.path.exists(self.save_data_large):
                        os.mkdir(self.save_data_large)
                if not os.path.exists(self.save_data_before):
                        os.mkdir(self.save_data_before)
                if not os.path.exists(self.save_data_after):
                        os.mkdir(self.save_data_after)
                if not os.path.exists(self.save_vert_data):
                        os.mkdir(self.save_vert_data)
                if not os.path.exists(self.save_vert_tip_pos):
                        os.mkdir(self.save_vert_tip_pos)
        ...
        def reset(self, 
                ...
                if scan_mol_x_nm is None or scan_mol_y_nm is None:
                        scan_mol_x_nm, scan_mol_y_nm, scan_mol_w_nm, scan_mol_h_nm, scan_mol_angle = self.nanonis_controller.Scan.FrameGet()
        ...
        def save_scan_image(
                ...
                angle: float = 0,
                pixel: int = 128,
                scan_voltage: float = 1.0,
                scan_current_offset: float = -1.0e-12,
                scan_current_cali: float = 1.0e-9,
                ...
                Args:
                        ...
                        x (_type_, optional): the center_x_nm of scanning region.
                        y (_type_, optional): the center_y_nm of scanning region.
                        len_w (_type_, optional): the length of scanning region.
                        len_h (_type_, optional): the length of scanning region.
                        angle (_type_, optional): the angle of scanning region.
                        scan_voltage (_type_, optional): the bias voltage of scanning, unit V.
                        scan_speed (_type_, optional): the speed of scanning, unit nm/s.
                        scan_direction (_type_, optional): the direction of scanning, 'down' or 'up'.
                        scan_current (_type_, optional): the current of scanning, unit A.
                ...
                self.nanonis_controller.Scan.BufferSet(pixels=pixel)
                self.nanonis_controller.Scan.SpeedSet(fwd_speed=scan_speed, bwd_speed=scan_speed)
                self.nanonis_controller.Scan.FrameSet(x, y, len_w, len_h,angle=angle)
                self.nanonis_controller.Bias.Set(scan_voltage)
                self.nanonis_controller.Current.CalibrSet(calibration=scan_current_cali, offset=scan_current_offset)
                self.nanonis_controller.Scan.Action('start',scan_direction=scan_direction)
                scan_data=self.nanonis_controller.Scan.FrameDataGrab(0, 1)[1]
                self.nanonis_controller.Scan.WaitEndOfScan()


                if save_img:
                        plt.imsave('%s/img_forward_%s.png' % (save_img_folder, img_name), scan_data)
        ...
        def tip_form_region(
                ...
                tip_form_x, tip_form_y, tip_form_z, tip_form_ref_x, tip_form_ref_y, tip_form_len_nm, find_tip_region=float(f'{tip_form_x}e-9'), 
                ...
                nano_control.FolMe.XYPosSet(9.6e-8, 9.0e-8, Wait_end_of_move=True)
                nano_control.TipShaper.PropsSet(Tip_lift=-2.5e-9, Bias_lift=2, Restore_feedback=1)
                nano_control.TipShaper.Start(wait_until_finished=1,timeout=10)
        ...
        def get_state(self,
                ...
                save_img_large='%s/%s_%s' % (task_folder, self.save_scan_img_large_prefix, current_time)
                save_img_before='%s/%s_%s' % (task_folder, self.save_scan_img_before_prefix, current_time)
                save_img_after='%s/%s_%s' % (task_folder, self.save_scan_img_after_prefix, current_time)
                save_data_large='%s/%s_%s' % (task_folder, self.save_scan_data_large_prefix, current_time)
                save_data_before='%s/%s_%s' % (task_folder, self.save_scan_data_before_prefix, current_time)
                save_data_after='%s/%s_%s' % (task_folder, self.save_scan_data_after_prefix, current_time)
                self.get_state_current_time=current_time
